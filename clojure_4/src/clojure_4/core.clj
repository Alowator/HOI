...
(defn clear_buffer [buff]
  (reduce-kv (fn [acc k _] (assoc acc k 0)) {} buff))
...
(defn notify-msg
  ...
  (let [updated_buffer (if (> ((state :bill) ware) ((state :buffer) ware))
                 (try
                   (do
                     (swap! storage-atom (fn [x] (- x (min (- ((state :bill) ware) ((state :buffer) ware)) amount))))
                     (update (state :buffer) ware (fn [x] (+ x (min (- ((state :bill) ware) x) amount)))))
                   (catch IllegalStateException _ (state :buffer)))
                 (state :buffer))]
    (assoc state
      :buffer (if (reduce-kv (fn [acc k v] (and acc (= v ((state :bill) k)))) true updated_buffer)
                   (do
                     (send ((state :target-storage) :worker) supply-msg (state :amount))
                     (clear_buffer (state :buffer)))
                   updated_buffer))))
